...
def test_resolve_csharp_helpers_expands_easy_text(tmp_path):
    cfg = _config(tmp_path)
    helper_dir = tmp_path / "src" / "tflex_harness" / "csharp_helpers"
    helper_dir.mkdir(parents=True)
    for name in ["TFlexEasyUnits.cs", "TFlexEasyDiagnostics.cs", "TFlexEasySession.cs", "TFlexEasyPrototype.cs", "TFlexEasyText.cs"]:
        (helper_dir / name).write_text("namespace TFlexEasy {}", encoding="utf-8")

    helpers, unknown = resolve_csharp_helpers(cfg, ["easy_text"])
...
def test_resolve_csharp_helpers_expands_easy_document_properties(tmp_path):
    cfg = _config(tmp_path)
    helper_dir = tmp_path / "src" / "tflex_harness" / "csharp_helpers"
    helper_dir.mkdir(parents=True)
    for name in [
    ...
    helpers, unknown = resolve_csharp_helpers(cfg, ["easy_document_properties"])
