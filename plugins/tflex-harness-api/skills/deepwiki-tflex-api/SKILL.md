...
- Prefer the local docs checkout from `TFLEX_API_DOCS_DIR` for exact grep/search when present.
- If `TFLEX_API_DOCS_DIR` is unset, try a sibling checkout named `tflex_api` next to the harness repo.
