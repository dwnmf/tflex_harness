...
Create a sheet-metal fragment document from the installed fragment prototype by copying the prototype, setting a writable document property, saving a new `.grb`, reopening it, and verifying persistence.
...
Snippet: `agent_workspace/recipes/create_fragment_sheet_metal_part_from_prototype.cs`
...
prototype_id=<catalog id, defaults to Фрагменты/Листовая Деталь.grb>
prototype_selector=<catalog id/name/relative path alias, optional>
property_name=<writable string property, defaults to Title>
text_value=<new text, defaults to Harness Fragment Sheet Metal Part>
```

## Live Verification Report

Test: `create_fragment_sheet_metal_part_from_prototype`
...
- category default value: `Harness Fragment Sheet Metal Part`;
- stdout: `document.opened=True`;
- stdout: `documentProperty.exists=True`;
- stdout: `documentProperty.after.Title=Harness Fragment Sheet Metal Part`;
- stdout: `endChanges=OK`;
- stdout: `documentProperty.set=True`;
- stdout: `document.saved=True`;
- stdout: `document.outputSize=27593`;
- stdout: `documentProperty.reopened=Harness Fragment Sheet Metal Part`;
